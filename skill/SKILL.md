...
```
User: "Start expertflow on ghost, focus on speed optimisation"
Echo: 
  1. SSH to ghost
  2. tmux new-session -d -s lp-expertflow -c ~/dev/expertflow
  3. Start claude-code in the session
  4. Send task: "Optimise inference speed..."
  5. Set up 10-min monitoring cron
  6. Report: "🦞 Pot 'expertflow' running on ghost with Claude Code"

User: "How's expertflow going?"
Echo:
  1. SSH capture-pane lp-expertflow
  2. Analyse output
...
Echo:
  ...
  2. Confirm: "Sent direction to expertflow"
```
